...
group_taxa_donut_facet_wrap=function(phylo,group.varn,ranklv="Phylum",topn=5,facet.var=".",x_shift=3,...){
  ...
  if(all(!sample_variables(phylo)%in%facet.var)){
    facet.var="."
  }
  ...
  a4=sprintf("group_by(a3,%sx_var,rank_var)"
             ,ifelse(facet.var==".","",paste0(facet.var,",",collapse = "")))
  df=eval(parse(text=a4))%>%summarise(RA=mean(Abundance))

  d1=df

  d2=split(d1,d1[,c(facet.var,"x_var") %>% "["(.!=".")]) %>%
    lapply(function(x){
  ...
  if(facet.var!=".")
    p1=p1+facet_wrap(formula(sprintf("~%s",paste0(facet.var,collapse="+"))),...)
