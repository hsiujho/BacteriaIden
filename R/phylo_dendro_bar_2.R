...
phylo_dendro_bar_2=function(phylo,ranklv="Phylum",topn=6
  ...
  b1=phylo_dendrogram(phylo, ranklv=ranklv,method=hc_method,dist_method=dist_method)
  ...
  if(missing(group_var)){
    ...
    for(k in 1:length(group_var)){
      df=my_get_variable(phylo,group_var[k]) %>>%
        ...
        mutate(x1=x-bar_width,x2=x+bar_width
               ,y1=group_ymax-group_height*k-group_grid*(k-1)
               ,y2=group_ymax-group_height*(k-1)-group_grid*(k-1))

      df_col=data.frame(item=sort(unique(df[[group_var[k]]])),col_id=scales::hue_pal()(length(unique(df[[group_var[k]]]))),stringsAsFactors = F)
    ...
    group_y_posi=(group_ymax-(0:length(group_var))*group_height) %>>%
...
function(){
  ...
  p2=phylo_dendro_bar_2(phylo,ranklv="Genus",topn=15,group_var = c("group","ST"))
